    ...
    for (auto it : values) {   //https://github.com/cocos2d/cocos2d-x/blob/v3/cocos/base/CCValue.h
        switch (it.second.getType()) {
            ...
            case cocos2d::Value::Type::FLOAT: {
                jclass jFloatClass = jniGetInstance.env->FindClass("java/lang/Float");
                jmethodID jValueOfId = jniGetInstance.env->GetStaticMethodID(jFloatClass,
                                                                             "valueOf",
                                                                             "(F)Ljava/lang/Float;");
                jobject jFloatValue = (jobject) jniGetInstance.env->CallStaticObjectMethod(
                        jFloatClass, jValueOfId, it.second.asFloat());

                jniGetInstance.env->CallObjectMethod(hashMapObj, hashMapId,
                                                     jniGetInstance.env->NewStringUTF(
                                                             it.first.c_str()), jFloatValue);

                jniGetInstance.env->DeleteLocalRef(jFloatValue);
            }
                break;
