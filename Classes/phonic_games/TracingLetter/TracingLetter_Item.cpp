...
TracingLetter_Item * TracingLetter_Item::createItem(int id, math::common::TouchPermission touch, math::resource::Image * image, std::string gaf_file_path, bool color)
{
	auto item = new TracingLetter_Item();
	if (item) {
		item->autorelease();
...
TracingLetter_Item::TracingLetter_Item()
	: _image(nullptr)
	, _background(nullptr)
	, super_node(nullptr)
	, _lbl_value(nullptr)
...
void TracingLetter_Item::didLoadFromCSB(int id, math::common::TouchPermission touch, math::resource::Image * image, std::string gaf_file_path, bool color)
{
...
void TracingLetter_Item::setItemSize(cocos2d::Size size)
{
...
void TracingLetter_Item::setImageColor(cocos2d::Color3B color)
{
	if (_image) {
...
void TracingLetter_Item::setImageVisible(bool visible)
{
	if (_image) {
...
void TracingLetter_Item::setBackgroundVisible(bool visible)
{
	if (_background) {
...
void TracingLetter_Item::setBackgroundColor(cocos2d::Color3B color)
{
...
void TracingLetter_Item::setGAFVisible(bool visible)
{
	if (_gaf) {
...
void TracingLetter_Item::setSuperNode(cocos2d::Node * node)
{
	super_node = node;
}

void TracingLetter_Item::onShow(float dt, math::animation::ObjectShowType show, int delta)
{
	this->stopAllActions();
	captureState();
...
void TracingLetter_Item::onShowGAF(float dt)
{
	if (_gaf) {
...
void TracingLetter_Item::onCorrectGAF(float dt)
{
...
void TracingLetter_Item::onHide(float delay_time)
{
...
void TracingLetter_Item::setColoring(bool coloring)
{
	_coloring = coloring;
}

bool TracingLetter_Item::isColoring()
{
	return _coloring;
}

bool TracingLetter_Item::isDetected(cocos2d::Vec2 position)
{
...
int TracingLetter_Item::getId()
{
	return _id;
}

void TracingLetter_Item::onTouchItem(bool correct, float delay, int opacity)
{
	scheduleOnce([=](float) {
		if (correct) {
...
bool TracingLetter_Item::isTouchedAt(cocos2d::Vec2 point)
{
...
void TracingLetter_Item::setTouchDetectedType(math::common::DetectedType detected_type)
{
	_detected_type = detected_type;
}

void TracingLetter_Item::initLabel(std::string text)
{
...
void TracingLetter_Item::setGAF(std::string gaf_file_path)
{
...
void TracingLetter_Item::setImage(math::resource::Image * image)
{
	if (!_image) {
...
void TracingLetter_Item::setBackground(math::resource::Image * background)
{
	if (!_background) {
...
void TracingLetter_Item::captureState() {
