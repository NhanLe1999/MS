...
Seapearl_Item * Seapearl_Item::createItem(CHARACTER_INFO item_info)
{
	auto item = new Seapearl_Item();
	if (item) {
		item->autorelease();
...
void Seapearl_Item::setDelegate(Seapearl_ItemDelegate * delegate)
{
	this->_delegate = delegate;
}


void Seapearl_Item::showFish()
{
	gaf_fish->playSequence("enlarge");
	lbl_info->runAction(cocos2d::FadeIn::create(13.0f / 32.0f));
}

void Seapearl_Item::hideFish()
{
}

void Seapearl_Item::enableTouch()
{
	m_touch_layout->setTouchEnabled(true);
}

void Seapearl_Item::disableTouch()
{
	m_touch_layout->setTouchEnabled(false);
}

cocos2d::ui::Text * Seapearl_Item::getLblInfo()
{
	return lbl_info;
}

void Seapearl_Item::setSwimArea(cocos2d::Rect rect)
{
	swim_area = rect;
}

void Seapearl_Item::setFlipped(bool flippedX)
{
...
GAFWrapper * Seapearl_Item::getGAFFish()
{
	return gaf_fish;
}


void Seapearl_Item::didLoadFromCSB(CHARACTER_INFO item_info)
{
	...
	gaf_fish = GAFWrapper::createGAF("gaf/seapearl/sp_fish/sp_fish.gaf");
	gaf_fish->setPosition(cocos2d::Vec2(30, 140));
	gaf_fish->playSequence("swim_slow");
	this->setName(item_info.character);
...
void Seapearl_Item::act(Seapearl_Item * target, std::function<void()> call_back)
{
...
void Seapearl_Item::seek(Vec2 target)
{
	Vec2 location = this->getWorldPosition();
