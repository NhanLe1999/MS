...
SpaceShip_Item * SpaceShip_Item::createMeteorite(METEORITE_INFO item_info) {
    SpaceShip_Item * obj = new SpaceShip_Item();
    if(obj->init())
    {
        obj->autorelease();
...
void SpaceShip_Item::didLoad_Meteorite(METEORITE_INFO item_info){
    ...
    click_layout->addClickEventListener(CC_CALLBACK_1(SpaceShip_Item::onClickItem, this));
    
    text = cocos2d::ui::Text::create(item_info.character, "fonts/KGWHATTHETEACHERWANTS.TTF", 100);
...
SpaceShip_Item * SpaceShip_Item::createShip() {
    SpaceShip_Item * obj = new SpaceShip_Item();
    if(obj->init())
    {
        obj->autorelease();
...
void SpaceShip_Item::didLoad_Ship(){
    item_gaf = GAFWrapper::createGAF(PHONIC_MANAGER->gaf_cache.at("ship"));
    this->addChild(item_gaf, 5);
    item_gaf->setPosition(Vec2(0, -170));
}

void SpaceShip_Item::onEnter(){
    cocos2d::ui::Layout::onEnter();
}

void SpaceShip_Item::playSequence(std::string sequence, bool looped, std::function<void()> callback){
...
void SpaceShip_Item::shoot(Vec2 pos_shoot, std::function<void()> callback){
    auto bullet = cocos2d::ui::ImageView::create("games/spaceship/spaceship_bullet.png");
...
void SpaceShip_Item::explosive(std::function<void()> callback){
...
void SpaceShip_Item::onChooseWrong(){
    text->setVisible(false);
}

void SpaceShip_Item::onClickItem(cocos2d::Ref *sender){
    if (delegate) {
        delegate->onItemClicked(this);
    }
}

void SpaceShip_Item::setDelegate(SpaceShip_ItemDelegate *delegate){
