...
GingerBreadMan_VM_Board * GingerBreadMan_VM_Board::createItem(mj::PHONIC_BOARD_INFO item_info) {
    GingerBreadMan_VM_Board * obj = new GingerBreadMan_VM_Board();
    if(obj->init())
    {
        obj->autorelease();
...
void GingerBreadMan_VM_Board::didLoadFromCSB(mj::PHONIC_BOARD_INFO item_info) {
    _item_info = item_info;
    screen_size = cocos2d::Director::getInstance()->getWinSize();
    board = cocos2d::ui::ImageView::create("games/breaktheegg/GingerBreadMan_VM_Board.png");
...
void GingerBreadMan_VM_Board::onEnter(){
    cocos2d::ui::Layout::onEnter();
    this->showBoard();
}

void GingerBreadMan_VM_Board::showBoard(){
    audio_helper->play("sounds/breaktheegg/GingerBreadMan_VM_Boardin.mp3");
...
void GingerBreadMan_VM_Board::showVideo(std::string video_path, cocos2d::Size video_size, cocos2d::Vec2 video_position, cocos2d::Node *parent){
#if (CC_TARGET_PLATFORM != CC_PLATFORM_WIN32 && CC_TARGET_PLATFORM != CC_PLATFORM_MAC)
    ...
    videoPlayer->addEventListener(CC_CALLBACK_2(GingerBreadMan_VM_Board::videoEventCallback, this));
...
void GingerBreadMan_VM_Board::videoEventCallback(Ref* sender, VideoPlayer::EventType eventType)
{
    switch (eventType) {
        ...
        case VideoPlayer::EventType::COMPLETED:
            ...
            this->scheduleOnce([=](float dt){
                //videoCompleted();
                audio_helper->play("sounds/breaktheegg/GingerBreadMan_VM_Boardout.mp3");
                board->runAction(Sequence::create(EaseBackIn::create(MoveTo::create(1.0f, Vec2(screen_size.width/2, screen_size.height+100))),DelayTime::create(0.3f), CallFunc::create([=]{
...
void GingerBreadMan_VM_Board::setDelegate(GingerBreadMan_VM_BoardDelegate *delegate) {
	_delegate = delegate;
}

void GingerBreadMan_VM_Board::showImage(std::string img_path, Vec2 img_position){
