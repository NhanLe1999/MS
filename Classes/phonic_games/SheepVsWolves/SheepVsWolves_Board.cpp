...
#define IMG_BOARD_1 "games/sheepvswolves/board_1.png"
#define IMG_BOARD_2 "games/sheepvswolves/board_2.png"
SheepVsWolves_Board * SheepVsWolves_Board::createBoard(sheepvswolves::BOARD_INFO info)
{
	auto board = new SheepVsWolves_Board();
	if (board && board->init()) {
...
void SheepVsWolves_Board::setInfo(sheepvswolves::BOARD_INFO info)
{
	m_board_info = info;
	m_img_->loadTexture(info.image_path);
}

sheepvswolves::BOARD_INFO SheepVsWolves_Board::getInfo() const
{
	return m_board_info;
}

void SheepVsWolves_Board::setDelegate(SheepVsWolves_BoardDelegate * delegate)
{
	_delegate = delegate;
}

void SheepVsWolves_Board::setClickedBoardEnabled(bool enabled)
{
	m_layout_touch->setTouchEnabled(enabled);
}

void SheepVsWolves_Board::didLoadFromCSB(sheepvswolves::BOARD_INFO info)
{
	m_board_info = info;
