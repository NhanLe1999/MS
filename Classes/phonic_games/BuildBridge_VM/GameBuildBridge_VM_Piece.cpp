...
GameBuildBridge_VM_Piece * GameBuildBridge_VM_Piece::createPiece(int idx) {
    auto p = new (std::nothrow) GameBuildBridge_VM_Piece();
...
bool GameBuildBridge_VM_Piece::initPiece(int idx) {
    if (!ui::Layout::init()) {
        return false;
    }
...
void GameBuildBridge_VM_Piece::onTapRight(std::function<void()> callback) {
...
void GameBuildBridge_VM_Piece::onTapWrong() {
...
void GameBuildBridge_VM_Piece::show() {
