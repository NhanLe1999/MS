...
GameBuildBridge_VM_Mask * GameBuildBridge_VM_Mask::createMask() {
    auto p = new GameBuildBridge_VM_Mask();
    if (p && p->init()) {
        p->autorelease();
        return p;
    }
...
bool GameBuildBridge_VM_Mask::init() {
    if (!ui::Layout::init()) {
        return false;
    }
...
void GameBuildBridge_VM_Mask::focus(double duration) {
    this->runAction(ActionFloat::create(duration, _mask_size, 300, [this](float v) {
        _mask->setRadius(v);
    }));
}

void GameBuildBridge_VM_Mask::unFocus(double duration) {
