...
//#include "MMReceiptVerifierAPI.h"
...
- (void)didPurchaseTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    ...
    NSData * data_receipt = [NSData dataWithContentsOfURL:receiptURL];
    NSString * receipt = [data_receipt base64EncodedStringWithOptions:0];
    
    if (receipt == nil) {
        NSError * err = [NSError errorWithDomain:RMStoreErrorDomain code:RMStoreErrorCodeUnableToCompleteVerification userInfo:nil];
        [self didFailTransaction:transaction queue:queue error:err];
        return;
    }
    mj::VerifyAppleAPI::call([self, queue, transaction](bool is_success) {
        if (is_success) {
            auto current_time = [[NSDate date] timeIntervalSince1970];
            [[NSUserDefaults standardUserDefaults] setInteger:int(current_time) forKey:@"ns.default.last.time.refreshed.receipt"];
            [self didVerifyTransaction:transaction queue:queue];
        } else {
            auto last_updated_time = [[NSUserDefaults standardUserDefaults] integerForKey:@"ns.default.last.time.refreshed.receipt"];
            auto current_time = [[NSDate date] timeIntervalSince1970];
            [[NSUserDefaults standardUserDefaults] setInteger:int(current_time) forKey:@"ns.default.last.time.refreshed.receipt"];
            
            if (current_time - last_updated_time < 3600 * 24) {
                NSError * err = [NSError errorWithDomain:RMStoreErrorDomain code:RMStoreErrorCodeUnableToCompleteVerification userInfo:@{NSLocalizedDescriptionKey : [NSString stringWithUTF8String: ""]}];
                [self didFailTransaction:transaction queue:queue error:err];
                return;
            }
...
- (void)addProduct:(SKProduct*)product
{
    if(product)
    {
        id key = product.productIdentifier;
        
        NSLog(@"RMStore key: %@", key);

        if (![_products objectForKey:key])
        {
            // contains object
